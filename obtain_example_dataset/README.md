...
- for completness the pdf files are stored here.
  ...
  Note SKAMPI is measureinig in 2 polarisation channels or in full Stokes (S0, S1, S2, S3) and with noise diode on (ND1) and off (ND0). In this example the spectra shows 2 polarisations in combinatioon with the noise diode. The units are in uncalibrated albertary units.
